    ...
    try:
        ...
        if connection.is_connected():
            ...
            if args.diagnose:
                ...
                # Show relevant tables first
                wsal_tables = [t for t in tables if 'wsal' in list(t.values())[0].lower()]
                if wsal_tables:
                    print("\nWP Security Audit Log tables:")
                    for table in wsal_tables:
                        print(f"  - {list(table.values())[0]}")
                
                # Show first 10 other tables
                other_tables = [t for t in tables if 'wsal' not in list(t.values())[0].lower()][:10]
                if other_tables:
                    print(f"\nOther tables (showing first 10 of {len(tables) - len(wsal_tables)}):")
                    for table in other_tables:
                        print(f"  - {list(table.values())[0]}")
                    if len(tables) - len(wsal_tables) > 10:
                        print(f"  ... and {len(tables) - len(wsal_tables) - 10} more tables")
                
                print("\nContinuing with activity monitoring...\n")
