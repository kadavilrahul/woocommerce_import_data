...
def diagnose_wsal_tables(cursor, table_prefix='wp_'):
    """Diagnose the structure of WP Security Audit Log tables"""
    print("\n--- WP Security Audit Log Table Diagnosis ---")
    
    # Check tables
    tables_to_check = [f"{table_prefix}wsal_occurrences", f"{table_prefix}wsal_metadata"]
    ...
    try:
        cursor.execute(f"SELECT DISTINCT alert_id FROM {table_prefix}wsal_occurrences ORDER BY alert_id")
...
def main():
    ...
    parser.add_argument('--domain', type=int, choices=[1, 2], required=False, help='Domain number (1 or 2)')
    parser.add_argument('--limit', type=int, required=False, help='Number of records to retrieve (1-1000)')
    parser.add_argument('--days', type=int, help='Show events from the last X days (command-line only)')
    ...
    if not args.domain or not args.limit:
        print("Running Activity Monitor...")
        print("Select domain to monitor:")
        print("1. nilgiristores.in")
        print("2. silkroademart.com")
        while True:
            try:
                args.domain = int(input("Select domain [1-2]: "))
                if args.domain in [1, 2]:
                    break
        ...
        while True:
            try:
                args.limit = int(input("Enter number of records to retrieve [1-1000]: "))
                if 1 <= args.limit <= 1000:
                    break
                print("Please enter a value between 1 and 1000")
            except ValueError:
                print("Please enter a valid number")

    # Initialize CSV file early to catch permission issues
    csv_file = None
    if args.csv:
        try:
            ...
            csv_path = os.path.join('data', args.csv)
    ...
    try:
        ...
        if connection.is_connected():
            ...
            # Order and limit (show most recent records)
            # Ensure limit is at least 1 and not excessively large
            limit = max(1, min(args.limit, 1000))  # Cap at 1000 records
            ...
            if not records:
                # Get total records count for better diagnostics
                cursor.execute(f"SELECT COUNT(*) as total FROM {wsal_occurrences}")
                total_records = cursor.fetchone()['total']
                
                # Get distinct event IDs for diagnostics
                cursor.execute(f"SELECT DISTINCT alert_id FROM {wsal_occurrences} ORDER BY alert_id")
                available_events = [str(e['alert_id']) for e in cursor.fetchall()]
                
                # Build detailed message
                message = "No activity records found matching criteria\n"
                message += f"- Total records in table: {total_records}\n"
                message += f"- Available event IDs: {', '.join(available_events)}\n"
                
                if args.days:
                    message += f"- Filtered for last {args.days} days\n"
                if args.event:
                    message += f"- Filtered for event ID: {args.event}\n"
                if args.user:
                    message += f"- Filtered for user: {args.user}\n"
                
                print(message)
                
                if csv_file:
                    csv_writer.writerow([message.replace('\n', ' ')])
                    csv_file.close()
                return
            ...
            if csv_file:
                csv_file.close()
                if records:
                    print(f"CSV export completed with {len(records)} records: data/{args.csv}")
                else:
                    print(f"Empty CSV created: data/{args.csv}")
